...
MaterialColor newColorSwatch(Color color, {bool opaque}) {
  ...
  return new MaterialColor(c.value, getMaterialColorValues(c));
}
...
Color getSwatchShade(Color c, int swatchValue) {
  final hsl = new colr.RgbColor(c.red, c.green, c.blue).toHslColor();
  final rgbResult =
      new colr.HslColor(hsl.h, hsl.s, 100 - swatchValue / 10).toRgbColor();
  return new Color.fromRGBO(rgbResult.r, rgbResult.g, rgbResult.b, 1.0);
}
